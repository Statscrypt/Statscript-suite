    ...
    try:
        ...
        if cmd_func:
            if parsed_cmd["command"] == "gen":
                result = cmd_func(
                    session,
                    gen_expression=parsed_cmd["gen_expression"],
                    **(
                        {"condition": parsed_cmd["condition"]}
                        if parsed_cmd["condition"]
                        else {}
                    ),
                )
                ...
                image_base64 = cmd_func(session, variables=parsed_cmd["variables"])
...
def start_repl_interactive():
    ...
    while True:
        try:
            ...
            _process_command(
                session, tokenizer, parser, user_input, lambda t, p: print(p)
            )
        ...
        try:
            ...
            try:
                ...
                if task == "EXECUTE" and payload:
                    _process_command(
                        session, tokenizer, parser, payload, send_json_message_func
                    )
                elif task == "EXIT":
                    break
                else:
            ...
            send_json_message_func(
                "error", f"An unexpected error occurred in JSON REPL: {e}"
            )
