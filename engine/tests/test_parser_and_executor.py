...
def test_run_list_with_condition(loaded_session):
    ...
    assert "3     4       b" in result
    assert "5     6       c" in result
    assert "1     2       a" not in result
...
def test_tokenizer_mismatch_error():
    ...
    tokenizer = Tokenizer()
    with pytest.raises(
        exceptions.SyntaxError, match="r\\(198\\); Unexpected character: @"
    ):
...
def test_run_use_file_not_found(session):
    ...
    with pytest.raises(
        exceptions.FileError, match="r\\(601\\); File not found at non_existent.csv"
    ):
...
def test_run_summarize_var_not_found(loaded_session):
    ...
    with pytest.raises(
        exceptions.VariableError,
        match="r\\(101\\); Variables \\['non_existent'\\] not found in the dataset.",
    ):
...
def test_run_list_var_not_found(loaded_session):
    ...
    with pytest.raises(
        exceptions.VariableError,
        match="r\\(101\\); Variables \\['non_existent'\\] not found.",
    ):
...
def test_run_list_invalid_condition_syntax(loaded_session):
    ...
    with pytest.raises(
        exceptions.SyntaxError, match="r\\(198\\); Error in 'if' condition:"
    ):
...
def test_run_graph_variable_not_found(loaded_session):
    ...
    with pytest.raises(
        exceptions.VariableError,
        match="r\\(101\\); Variables 'non_existent' or 'var2' not found in the dataset.",
    ):
        run_graph(loaded_session, ["non_existent", "var2"])
    with pytest.raises(
        exceptions.VariableError,
        match="r\\(101\\); Variables 'var1' or 'another_non_existent' not found in the dataset.",
    ):
...
def test_run_graph_non_numeric_variable(loaded_session):
    ...
    with pytest.raises(
        ...
        match="r\\(4\\); Variable 'var_str' is not numeric and cannot be plotted.",
    ):
